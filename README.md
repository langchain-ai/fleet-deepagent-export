...
| `LANGSMITH_HOST_URL` | no | Override the LangSmith API host (default `https://api.smith.langchain.com`). For self-hosted, set to your LangSmith hostname with `/api` appended. Example: if your LangSmith is at `https://langsmith.acme.com`, set this to `https://langsmith.acme.com/api`. |
| `HOST_LANGCHAIN_API_URL` | no | Override the OAuth broker host (default `https://api.host.langchain.com`). For self-hosted with OAuth-backed Fleet tools (Gmail, GitHub, etc.), set to your LangSmith hostname with `/api-host` appended. Example: `https://langsmith.acme.com/api-host`. Your self-hosted deployment must have Fleet enabled (`config.deployment.enabled: true` in helm values) — see the [self-hosted Fleet setup docs](https://docs.langchain.com/langsmith/deploy-self-hosted-full-platform). |
